...
class Hyperboloid(Manifold):
    ...
    def minkowski_length(self, u, keepdim=True):
        dot = self.minkowski_dot(u, u, keepdim=keepdim)
        return torch.clamp(dot, min=self.eps[u.dtype])
    ...
    def mobius_matvec(self, m, x, c, rev=True):
        u = self.logmap0(x, c)
        if rev:
            mu = u @ m.transpose(-1, -2)
        else:
            mu = m.transpose(-1, -2) @ u
    ...
    def lorenz_factor(self, x, c=1.0, dim=-1, keepdim=False):
        ...
        """
        x_norm = self.minkowski_length(x, keepdim=keepdim)
