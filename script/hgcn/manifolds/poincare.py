...
class PoincareBall(Manifold):
    ...
    # me add
    def mobius_pointwise_mul(self, w, x, c):
        x_norm = x.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)
        assert c > 0
        sqrt_c = c ** 0.5
        wx = w * x
        wx_norm = wx.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)
    ...
    def k2p(self, x, c):
        a = 1 - c * x.pow(2).sum(-1, keepdim=True) + 1e-5
        assert torch.min(a) > 0
        denom = 1 + torch.sqrt(1 - c * x.pow(2).sum(-1, keepdim=True) + 1e-5)
        return x / denom
    ...
    def lorenz_factor(self, x, c=1.0, dim=-1, keepdim=False):
        ...
        a = 1 - c * x_norm + 1e-5
        assert torch.min(a) > 0
        tmp = 1 / torch.sqrt(1 - c * x_norm + 1e-5)
        return tmp

    def curvature_map(self, x, c1, c2):
        # transform a vector x from curvature c1 to curvature c2
        #cc1 = c1.detach().clone()
        #cc2 = c1.detach().clone()
        a = c1/c2
        assert a > 0
