...
class Runner(object):
    def __init__(self):
        ...
        args.training_length = len(self.train_shots)
        ...
        self.order_factor = []
        for i in range(args.spatial_order):
            self.order_factor.append(i + 1)
    ...
    def run(self):
        dilated_edge_index_list = prepare_dilated_edge_index(data, self.order_factor, args.device)
        ...
        for epoch in range(1, args.max_epoch + 1):
            ...
            for t in self.train_shots:
                ...
                list_edge_index = dilated_edge_index_list[t]
                optimizer.zero_grad()
                z, c, z_ = self.model(list_edge_index, t, self.x)
                if args.use_htc == 0:
                    epoch_loss = self.loss(z, edge_index, c_=c)
                else:
                    epoch_loss = self.loss(z, edge_index, c_=c) + self.model.htc(z, c)
                #with torch.autograd.detect_anomaly():
                epoch_loss.backward()
                optimizer.step()
                epoch_losses.append(epoch_loss.item())
                self.model.update_hiddens_all_with(z_)

                

            # eval        
            self.model.eval()
            test_results = self.test(epoch, z_)
    ...
    def test(self, epoch, embeddings=None, c=1.0):
        auc_list, ap_list = [], []
        ...
        for t in self.test_shots:
            ...
            auc, ap = self.loss.predict(embeddings, pos_edge, neg_edge, c_=c)
            auc_new, ap_new = self.loss.predict(embeddings, new_pos_edge, new_neg_edge, c_=c)
            auc_list.append(auc)
            ap_list.append(ap)
...
if __name__ == '__main__':
    ...
    from script.inits import prepare, prepare_dilated_edge_index
    import warnings
